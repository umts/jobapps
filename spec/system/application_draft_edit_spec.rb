...
describe 'editing application draft' do
  ...
  context 'viewing page' do
    ...
    it 'has a field to create a new question' do
      expect(find_by_id('fields-container').find_all('.row.field-row').count).to be 4
      # there are 3 existing questions, the 4th textarea field is for a new one
      expect(find_by_id('fields-container').find_all('.row.field-row textarea').last.value).to be_empty
  ...
  context 'clicking save changes and continue editing button' do
    it 'will add new questions to the draft' do
      ...
      field = find_by_id('fields-container').find_all('textarea')[3][:name]
      fill_in(field, with: 'Stuff')
      # select a data type for the new question
      select 'text', from: find_by_id('fields-container').find_all('select').last[:name]
