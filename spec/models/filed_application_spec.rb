...
describe FiledApplication do
  ...
  describe 'email_subscribers' do
    let(:record) { create :filed_application }
  ...
  describe 'between' do
    before :each do
      Timecop.freeze 1.week.ago do
        @too_past_record = create :filed_application
      end
      Timecop.freeze 1.month.since do
        @too_future_record = create :filed_application
      end
      Timecop.freeze 1.week.since do
        @almost_too_future_record = create :filed_application
      end
      @just_right_record = create :filed_application
      @start_date = Time.zone.today
      @end_date = 1.week.since
    end
    let :call do
      FiledApplication.between @start_date, @end_date
    end
  ...
  describe 'in_department' do
    ...
    let :call do
      FiledApplication.in_department @department.id
    end
  ...
  describe 'hire_count' do
    before :each do
      record_1 = create :filed_application
      record_2 = create :filed_application
      create :interview, filed_application: record_2, hired: false
      create :interview, filed_application: record_1, hired: true
    end
    let :call do
      FiledApplication.hire_count
    end
  ...
  describe 'interview_count' do
    before :each do
      record_1 = create :filed_application
      create :filed_application
      create :interview, filed_application: record_1
    end
    let :call do
      FiledApplication.interview_count
    end
  ...
  describe 'pending?' do
    it 'returns true if record has not been reviewed' do
      record = create :filed_application, reviewed: false
      expect(record).to be_pending
    end
    it 'returns false if record has been reviewed' do
      record = create :filed_application, reviewed: true
  ...
  describe 'move_to_dashboard' do
    let(:call) { FiledApplication.move_to_dashboard }
  ...
  describe 'self.combined_eeo_data' do
    before :each do
      @records = FiledApplication.all
      stub_const 'FiledApplication::ETHNICITY_OPTIONS', ['Klingon']
      stub_const 'FiledApplication::GENDER_OPTIONS', ['Other']
    end
    let :call do
      FiledApplication.combined_eeo_data @records
    end
    it 'calls AR#interview_count to count interviews' do
      expect(FiledApplication).to receive(:interview_count)
        .at_least(:once)
      call
    end
    it 'calls AR#hire_count to count hirees' do
      expect(FiledApplication).to receive(:hire_count)
        .at_least(:once)
      call
    end
    it 'counts only records and interviews with both ethnicity and gender' do
      create :filed_application, ethnicity: nil, gender: nil
      create :filed_application, ethnicity: '', gender: ''
      expect(call).to eql('Other' => [['Klingon', 0, 0, 0]])
    end
    it 'counts records/interviews whose ethnicity is not one of the options' do
      create :filed_application, ethnicity: 'Romulan', gender: 'Other'
      expect(call).to eql('Other' => [['Klingon', 0, 0, 0],
                                      ['Romulan', 1, 0, 0]])
    end
    it 'counts records/interviews whose gender is not one of the options' do
      create :filed_application, ethnicity: 'Klingon', gender: 'Male'
      expect(call).to eql('Other' => [['Klingon', 0, 0, 0]],
                          'Male' => [['Klingon', 1, 0, 0]])
    end
    it 'counts records/interviews where gender and ethnicity not in options' do
      create :filed_application, ethnicity: 'Romulan', gender: 'Male'
      expect(call).to eql('Other' => [['Klingon', 0, 0, 0],
                                      ['Romulan', 0, 0, 0]],
                          'Male' => [['Klingon', 0, 0, 0],
                                     ['Romulan', 1, 0, 0]])
  ...
  describe 'self.ethnicity_eeo_data' do
    ...
    let :call do
      FiledApplication.ethnicity_eeo_data @records
    end
    it 'calls AR#interview_count to count interviews' do
      expect(FiledApplication).to receive(:interview_count)
        .at_least(:once)
    ...
    it 'counts only records and their interviews with an ethnicity attribute' do
      create :filed_application, ethnicity: nil
      create :filed_application, ethnicity: ''
  ...
  describe 'self.gender_eeo_data' do
    ...
    before :each do
      @records = FiledApplication.all
      stub_const 'FiledApplication::GENDER_OPTIONS', ['Female']
    end
    let :call do
      FiledApplication.gender_eeo_data @records
    end
    it 'calls AR#interview_count to count interviews' do
      expect(FiledApplication).to receive(:interview_count)
        .at_least(:once)
  ...
  describe 'self.eeo_data' do
    ...
    it 'calls AR#ethnicity_eeo_data to populate hash with ethnicity numbers' do
      expect(FiledApplication).to receive(:ethnicity_eeo_data)
        .with(@relation)
    ...
    it 'calls AR#gender_eeo_data to assign values to genders key in hash' do
      expect(FiledApplication).to receive(:gender_eeo_data)
        .with(@relation)
    ...
    it 'calls AR#combined_eeo_data to populate hash with combo numbers' do
      expect(FiledApplication).to receive(:combined_eeo_data)
        .with(@relation)
