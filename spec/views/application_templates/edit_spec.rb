...
describe 'application_templates/edit.haml' do
  ...
  it 'has a prompt that is required' do
    render
    expect(rendered).to have_tag 'textarea', with: { required: 'required', name: 'question[prompt]' }
  end
  ...
  it 'contains a check box to determine whether question is required' do
    render
    expect(rendered).to have_tag 'input', with: { type: 'checkbox', name: 'question[required]' }
  end
